...
- E-mail: inlog-/systeemmails gaan via `MAIL_FROM` (`noreply@slimescore.com`); contact via `CONTACT_EMAIL`. Gebruik Cloudflare Email Routing voor inkomende alias-forwarding en stel Supabase Auth custom SMTP in via Resend voor uitgaande loginmails. Zet SPF/DKIM/DMARC klaar in Cloudflare.
