-- 1) Mededelingen ook op de live-subsite (live.slimescore.com).
alter table public.site_messages drop constraint if exists site_messages_placement_check;
alter table public.site_messages
  add constraint site_messages_placement_check
  check (placement in ('home', 'voorspellingen', 'live'));

-- 2) Per-speler invulvoortgang als server-side aggregate (geen full-table fetch,
--    dus geen PostgREST max-rows-cap). Alleen via service-role gelezen in /admin.
--    security_invoker zodat RLS van de onderliggende tabellen van toepassing blijft.
...
-- Geen toegang voor spelers; alleen de service-role (admin) leest dit.
